...
def reformat_config(df, exp_id, hpc_base_path, suffix):
    ...
    for index, row in df.iterrows():
        ...
        print(row['Sample Number'])
        arr = row['Sample Number'].split('_')
        if len(arr) == 1:
            wantedfilename=f'{arr[0]}_S'
        else:
            idn = arr[1]
            idname = arr[0]
            wantedfilename = f'{idname}_{idn}_S'

        result = list(filter(lambda x: x.startswith(wantedfilename) or x.replace('-','_').startswith(wantedfilename), filenames))
...
if __name__ == "__main__":
    try:
        ...
        # # debug 
        # hpc_base_path = "X:/projects/p073_GeneEditingSupport/Kim/09282023_293T_HK2_BE/output"
        # exp_id='09282023_293T_HK2_BE'
        # suffix='vs1'
        # nameinputfile='20230928_BE_input_form'
        # loglevel='DEBUG'
        ...
        logger.error("The pre-processing has failed. Kindly check.", exc_info=True)